...
# The default setup assumes Kind is pre-installed and builds/loads the controller Docker image locally.
# CertManager is installed by default; skip with:
# - CERT_MANAGER_INSTALL_SKIP=true
KIND_CLUSTER ?= vpa-creator-test-e2e
...
# PLATFORMS defines the target platforms for the controller image be built to provide support to multiple
...
docker-buildx: ## Build and push docker image for the controller for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name vpa-creator-builder
	$(CONTAINER_TOOL) buildx use vpa-creator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm vpa-creator-builder
	rm Dockerfile.cross
